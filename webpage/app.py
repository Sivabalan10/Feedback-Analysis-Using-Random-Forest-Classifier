...
def analytics():
    with sqlite3.connect('config.db') as conn:
        c = conn.cursor()
        c.execute("SELECT insights FROM feedback")
        insights = c.fetchall()
        
        c.execute("SELECT comment, insights FROM feedback")
        comments = c.fetchall()
    
    good_quality_count = sum(1 for i in insights if i[0] == "Good Quality")
    poor_quality_count = sum(1 for i in insights if i[0] == "Poor Quality")
    total_count = len(insights)
    overall_prediction = "Good Quality" if good_quality_count > poor_quality_count else "Poor Quality"
    recommendation = "You are having great feedback, congrats."
    if overall_prediction == "Poor Quality":
        recommendation = "Consider improving event organization and management."
